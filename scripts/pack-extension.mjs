 ...
 * pack-extension.mjs
 *
 * 将 extension/ 打成可上传 Chrome Web Store 的 zip。
 * 产物名: free-ig-export-v<version>.zip (与 manifest.json 的 version 同步)
 *
...
async function main() {
  const manifestRaw = await fs.readFile(manifestPath, 'utf-8').catch((err) => {
    console.error('[pack] 读取 extension/manifest.json 失败:', err.message);
    process.exit(1);
  });
  const { version } = JSON.parse(manifestRaw);
  const zipName = `free-ig-export-v${version}.zip`;
