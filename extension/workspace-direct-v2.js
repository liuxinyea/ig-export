...
$("#start").addEventListener("click", async () => {
  ...
  try { await FreeIgExportLicense.requireCollection(Math.max(1, Number($("#limit").value) || 100)); }
  catch (error) { return showError(error.message); }
  await ensureAvatarPermission();
  if (activeRun) activeRun.stopped = true;
  page = 1;
  const task = { id: crypto.randomUUID(), status: "starting", reason: null, sourceProfile: username, listType: $("#listType").value, limit: Math.max(1, Number($("#limit").value) || 100), delayMs: Math.max(1000, Number($("#delay").value) || 5000), records: [], nextCursor: "", profileId: "", createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() };
...
async function runCollection(initial) {
  ...
  try {
    ...
    while (!run.stopped && records.length < task.limit) {
      ...
      if (run.stopped || run.paused) continue;
      ...
      if (run.stopped || current?.id !== run.id) break;
      records.push(...incoming);
      cursor = result.nextCursor || "";
      task = { ...task, status: run.paused ? "paused" : "running", reason: null, profileId, sourceProfile: username, records, nextCursor: cursor, updatedAt: new Date().toISOString() };
      await saveTask(task);
