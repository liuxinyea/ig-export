...
- `background.js` — message router; owns the single task object in `chrome.storage.local` under key `free-ig-export.currentTask`, dedupes records by username, fetches avatars.
...
- Message protocol: `free-ig-export:<action>` over `chrome.runtime.sendMessage`; async listeners must `return true`.
